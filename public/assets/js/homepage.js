document.addEventListener("DOMContentLoaded", () => {
  ...
  if (roomsGrid && navArrowLeft && navArrowRight) {
    ...
    const updateCardElevation = () => {
      ...
      const firstVisibleCardIndex = Math.round(
      ...
      if (cards[firstVisibleCardIndex + visibleCardsCount - 1]) {
        cards[firstVisibleCardIndex + visibleCardsCount - 1].classList.add(
          "elevated"
        );
      }
    };
    ...
    // test để xem kích thước của roomCard
    // if (roomCard) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateCarousel() {
    ...
    nextBtn.style.opacity =
      currentPosition >= roomContainer.scrollWidth - roomContainer.clientWidth
        ? "0.3"
        : "0.6";
  }
